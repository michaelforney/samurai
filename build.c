	...
	for (; *fmt; ++fmt) {
		if (*fmt == '%') {
			...
			case 'u':
				...
				break;
			case 'p':
				printf("%3lu%%", 100 * nfinished / ntotal);
				break;
			case 'e':
				clock_gettime(CLOCK_MONOTONIC, &ctime);
				printf("%.3f",
					(ctime.tv_sec + ctime.tv_nsec * 0.000000001) -
					(starttime.tv_sec + starttime.tv_nsec * 0.000000001));
				break;
			case '%':
				putchar('%');
				break;
...
build(void)
{
	...
	if ((ninja_status = getenv("NINJA_STATUS"))) {
		fmt = ninja_status;
		for (; *fmt; ++fmt) {
			if (*fmt == '%') {
				switch (*(++fmt)) {
